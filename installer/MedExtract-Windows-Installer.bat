...
title MedExtract Installer v1.0.0
color 0B
...
echo.
echo        MedExtract Windows Installer
echo        Version 1.0.0
echo ============================================
echo.
...
powershell -Command "[Net.ServicePointManager]::SecurityProtocol='Tls12'; (New-Object Net.WebClient).DownloadFile('https://github.com/sobhi-jabal/MedExtract-Official/archive/refs/heads/main.zip', '%TEMP%\medextract.zip')"
...
:: Move files
if exist "%TEMP%\MedExtract-Official-main" (
    xcopy "%TEMP%\MedExtract-Official-main\*" "%INSTALL_DIR%\" /E /H /Y /Q >nul
    rmdir /s /q "%TEMP%\MedExtract-Official-main" 2>nul
) else (
...
if not errorlevel 1 (
    echo     Local Ollama installation detected
...
if "%USE_LOCAL_OLLAMA%"=="YES" (
    ...
    echo     Using containerized Ollama configuration
    if "%CORPORATE%"=="YES" (
...
echo   - MedExtract.bat (start application)
echo   - Stop-MedExtract.bat (stop application)
echo.
if "%USE_LOCAL_OLLAMA%"=="YES" (
